...
const { getLegisInfoCaller } = require('../getDataCallers');
...
describe('getLegisInfoCaller', () => {
  ...
  test('getLegisInfoCaller should return two arrays, given a correct URL', async () => {
    const { formattedBillsArray, eventsArray } = await getLegisInfoCaller(
      testUrl
    );
    expect(formattedBillsArray).toBeInstanceOf(Array);
